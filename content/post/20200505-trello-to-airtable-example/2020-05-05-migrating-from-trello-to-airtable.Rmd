...
authors: [admin]
lastmod: '2020-05-06T21:47:11-04:00'
featured: yes
draft: yes
image:
  caption: ''
...
![](/post/2020-05-05-migrating-from-trello-to-airtable/trello-to-airtable-main.png)
...
> Shout out to the [Silicon Valley Democratic Socialists of America!](https://siliconvalleydsa.org/) who provided inspiration and support! Their mutual aid network is extensive and functions at an impressive scale. Get connected with the South Bay Mutual Aid network here: https://www.southbay.aidnetwork.org/
...
# What to expect
The code provided in the R Markdown file linked above requires the `tidyverse` and `JSONlite` packages as well as a JSON file with your Trello data. It will generate the following files.

- _tidy_cards.csv_ file ready to import to Airtable 
- _attachments_ folder containing any content you've attached to your cards using an URL. 
- _attachment_errors.csv_ file containing the card name, or task, as well as the URL/file path of any attachments that failed to download (i.e. links to files stored locally), so that you exactly which links you'll need to locate manually.
...
- Rendered markdown from your card descriptions. Airtable does support markdown in its "long description" field type, but it doesn't automatically render from imported text.

# Choose your own adventure
1. **Download the R Markdown file** linked above and run with it. Jump down to section "Tidying in Airtable" for pointers on how to get your Airtable base configured like your Trello board.
2. **Keep reading** for a codethrough of the R Markdown file, using the example featured in the picture above, with tips and resources along the way.
...
Last but not least -- we'll be keeping things tidy, with the help of the `tidyverse` package, to ensure each variable is in its own column, observations are in rows, and values are in cells. You can read more about tidy data in [Chapter 12 of R for Data Science](https://r4ds.had.co.nz/tidy-data.html).
...
## Tasks vs Subtasks
- There are probably many ways to creatively group your records to parallel the checklist option within a Trello card moving forward. One is to think of your primary field `Tasks` instead as **'subtasks'** and create a new column to serve as the umbrella **'task'**, with a "single selection" field type. Then you could group your records by **'task'**.
