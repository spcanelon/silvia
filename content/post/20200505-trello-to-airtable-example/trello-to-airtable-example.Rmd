...
summary: Tutorial on how to migrate your current Trello board to an Airtable base from start to finish. #important to avoid unformatted text on the home page preview
draft: true
featured: true
...
My exposure to working with databases began the summer after I finished graduate school by way of a free two-part Microsoft Office Access community class hosted my public library. In looking for a database tool that I could access on my smartphone, I came across Airtable -- a user-friendly and _powerful_ tool that I started using for personal projects (i.e. document organizing, apartment hunting, etc.). Since then I've received more formal training in relational databases for my research role, and have felt more comfortable exploring more advanced features of airtable. Recently I leaned on Airtable to support the [Philadelphia Reproductive Freedom Collective](https://www.womensmedicalfund.org/prfc) and created a base (with some inspiration and knowledge-sharing with the [Silicon Valley Democratic Socialists of America!](https://siliconvalleydsa.org/)) to track our COVID-19 mutual aid efforts.

Having fallen in some Airtable deep-work I figured maybe it was time to **retire my Trello boards** in favor of some task bases. When I went to export my Trello data I noticed my free account didn't let me download the data into a CSV format. The only options available were to print as a PDF or export as JSON! Anyway, I figured this would be a good opportunity to learn about this data format so that I could get it into a CSV that I could upload to Airtable. Nothing like spending hours tinkering in R for the sake of _future_ project management...to the disregard of _present_ project management. That's how I chose to spend my weekend, but that doesn't mean you have to. **Cue code-through.**
...
Kan Nishida's [Working with JSON data in a very simple way](https://blog.exploratory.io/working-with-json-data-in-very-simple-way-ad7ebcc0bb89) is a fantastic post that I went back to time and time again.

## Cards

*Flattening* using `flatten()` makes the nested hiearchical data structure into a flatter structure by assigning each of the nested variable as its own column as much as possible.
